    ...
    return (
        <main >
            ...
            {subjects.map((subject, index) => (
                <Subject key={index} name={subject.name} group={GroupMap[index]} score={subject.score} bgColor={ColorMap[index]} color={index === 1 || index === 3 ? "white" : "black"}/>
            ))}   
        </main>
    );
}

function Subject({ name, group, score, bgColor, color }) {
    return (
